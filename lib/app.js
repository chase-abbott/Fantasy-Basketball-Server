...
import { getPlayers, getNews, mungeNews, mungePlayers, getScores, mungeScores, getVids, mungeVids } from '../utils/munge-functions.js';
//comment
...
app.post('/api/me/players', async (req, res) => {
  try {
    ...
    `, [player.playerId, player.name, player.position, player.fantasyPoints, player.userId]);
...
app.delete('/api/me/players/:userId', async (req, res) => {
  try{
    const data = await client.query(`
...
app.delete('/api/me/team/:userId', async (req, res) => {
  try{
