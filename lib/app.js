...
app.get('/api/vids', async (req, res) => {

  try {
    const vids = await getVids();
    const mungedVids = mungeVids(vids);
...
app.put('/api/me/team/:id', async (req, res) => {
  try {
    ...
    const data = await client.query(`
      ...
      and id = $5
      RETURNING id, team, starting_five as "startingFive", bench, user_id as "userId";`
    , [team.team, team.startingFive, team.bench, req.userId, req.params.id]);
...
// id here is user id, intent is to delete all players associated w/ user
// just shorten the route and it almost reads like English: delete my(me) players.
// not safe to pass the userId like that (I know your SQL would prevent because of req.userId), 
// don't fool yourself that this is okay
app.delete('/api/me/players', async (req, res) => {
  try{
    const data = await client.query(`
      DELETE FROM userPlayers
      WHERE user_id = $1
      -- AND user_id = $2  // identical column and criteria, big clue something is off
      RETURNING id, player_id as "playerId", name, position, fantasy_points as "fantasyPoints", user_id as "userId";
    `,
    [req.userId]);
...
// id here is team id created by db, intent is to delete user's team
// again, id not needed and not safe
app.delete('/api/me/team', async (req, res) => {
  try{
